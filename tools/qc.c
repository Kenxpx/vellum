/*
 * qc.c - command-line compiler for the Vellum source language.
 *
 * Usage: qc <in.ql> [out.qbc]
 * Reads the whole source file into memory, compiles it into a .qbc module,
 * and writes the bytes to the output path. When no output path is given it is
 * derived from the input by replacing its extension with ".qbc". On a compile
 * failure the message from vl_compile_source is printed to stderr.
 */
#include <stdio.h>
#include <stdlib.h>
...
/* Read the entire source file at path into a freshly allocated buffer. */
...
/* Entry point: read source, compile, and write the resulting module. */
int main(int argc, char **argv)
{
    ...
    if (argc < 2) {
        fprintf(stderr, "usage: qc <in.ql> [out.qbc]\n");
        return 1;
    }
    ...
    errbuf[0] = '\0';
    st = vl_compile_source(text, text_len, &out, &out_len, errbuf, sizeof(errbuf));
    vl_free(text);
    if (st != VL_OK) {
